...
class CameraThread(QThread):
    ...
    def run(self):
        while self.running:
            ...
            slope_above, intercept_above, r_value_above, p_value_above, std_err_above = linregress(y_above, z_above)
            slope_below, intercept_below, r_value_below, p_value_below, std_err_below = linregress(y_below, z_below)
            # Create the regression line
            z_pred_above = slope_above * y_above + intercept_above
            z_pred_below = slope_below * y_below + intercept_below
            
            # Calculate the angle between the lines
            angle_radians = np.arctan(abs((slope_below - slope_above) / (1 + slope_below * slope_above)))
            angle_degrees = np.degrees(angle_radians)
            print(f"Angle between the regression lines: {angle_degrees:.2f} degrees")
            
            # Optional: Plot the results (requires matplotlib)
            import matplotlib.pyplot as plt

            plt.scatter(y_above, z_above, label="above Data Points")
            plt.scatter(y_below, z_below, label="below Data Points")
            plt.plot(y_above, z_pred_above, color='red', label="above Regression Line")
            plt.plot(y_below, z_pred_below, color='blue', label="below Regression Line")
            plt.xlabel("y")
            plt.ylabel("z")
            plt.axis('equal')
            plt.grid(True)
            plt.legend()
            plt.title("Linear Regression of y vs z")
            plt.show()
